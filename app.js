      ...
      function handleActionClick(el) {
        ...
        if (action === "toggle-random") state.isRandomOn = !state.isRandomOn;
        if (action === "prev") safeSetIndex(-1, true);
        if (action === "next") {
          if (state.isRandomOn) {
            safeSetIndex(getAdvanceIndex(), false);
          } else {
        ...
        if (action === "toggle-autoplay") toggleAutoplay();
        if (action === "pick") {
                ...
                <main id="main-panel" class="rounded-3xl bg-white p-5 shadow-sm md:p-8">
                  ...
                  <div class="min-h-[330px] rounded-3xl bg-slate-100 p-6 md:p-10">
                    <div class="min-h-[148px] overflow-visible rounded-2xl bg-white/50 p-3 pb-5 md:min-h-[172px] md:p-4 md:pb-6">
                      <p id="english-display" class="text-3xl font-extrabold leading-normal tracking-tight pb-1 md:text-5xl">${buildTypingMarkup(current.english, state.answer)}</p>
                    </div>
                    <div class="mt-4 max-h-[64px] overflow-hidden rounded-2xl bg-white/40 p-1.5">
                      <div class="flex flex-wrap gap-2">${buildPronunciationMarkup(current.english)}</div>
                    </div>
