...
- 코드 변경사항이 발생할 때마다 해당 변경을 즉시 커밋합니다.
...
- 카드 모드 상단 `#번호` 배지 오른쪽에서 영어 문장 표시/숨김 토글을 지원하며, 토글할 때마다 입력 중이던 답안과 피드백을 초기화합니다. 숨김 상태에서는 문장 영역에 숨김 안내 문구만 표시하고, 발음 칩은 레이아웃을 유지한 채 글자만 보이지 않게 처리하며, 타이핑 중 정타/오타 하이라이트를 표시하지 않습니다. 우측 사이드바 예문 리스트는 레이아웃을 유지한 채 글자만 보이지 않게 처리합니다.
