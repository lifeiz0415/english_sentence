...
- 버튼 지원: previous/next, 랜덤 on/off 토글, 자동재생 on/off 토글, 듣기 on/off 토글, 진도 초기화.
- 암기완료(`mastered=true`) 문장은 학습 대상/사이드바 목록에서 제외되어 다시 나오지 않습니다. 단, 전체 문장을 모두 암기하면 전체 목록을 다시 순환합니다.
- 앱 시작 기본값: 랜덤 on, 듣기 on.
- 사이드바는 전체 문장 인덱스 선택을 지원합니다.
